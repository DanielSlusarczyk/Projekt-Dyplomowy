...
class WData:
    ...
    def get_scaled(self, columns) -> pd.DataFrame:
        scaler = MinMaxScaler()

        df_scaled = pd.DataFrame(scaler.fit_transform(self.data[columns]), columns=columns)
        df_scaled['DateTime'] = self.data['DateTime']
